...
def lookup_plant(plant_name: str) -> dict:
    ...
    # 1. Direct key match — O(1) dict access, fastest path.
    if normalized in _plant_db:
        return {"found": True, "plant": _plant_db[normalized]}

    # 2. Display name match, then 3. alias match.
    ...
    return {
        "found": False,
        "name": normalized,
        "message": "Plant not found in the database. Try a common name, scientific name, or known alias.",
    }
